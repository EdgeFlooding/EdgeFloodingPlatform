...
def consume(id_gpu, id_this_node, detector, fs_list, run_event, logger, ip_address_cloud, yolo_run):
    ...
    with tf.device('/device:GPU:' + str(id_gpu)):
        while not run_event.is_set():
            ...
            logger.info(f'[INFERENCE] {{"ID": {frame_object.id}, "FRAMESLOT": {frame_object.id_slot}, "CREATION_TS": {frame_object.creation_timestamp}, "SERVICE_TS": {frame_object.service_timestamp}, "COMPLETION_TS": {frame_object.completion_timestamp}, "ID_GPU": {id_gpu}}}')
...
def start_server(fs_list, logger, n_cameras):
    ...
    # listen on port 5005
    print('Starting server. Listening on port all ports from 5001 to 5040.')
